...
@app.route("/api/employer/issuance-request", methods = ['GET'])
def employerIssuanceRequest():
    ...
    if "claims" in payload is not None:
        ...
        payload["claims"]["email_address"] = "TDurden@PaperStreetSoap.Com"
...
@app.route("/api/employer/issuance-request-callback", methods = ['POST'])
...
@app.route("/api/employer/issuance-response", methods = ['GET'])
