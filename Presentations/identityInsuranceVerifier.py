...
if presentationFile is None:
    presentationFile = os.path.realpath(os.path.join(os.path.dirname(__file__), '../Config/identity-insurance_presentation.json'))
...
identity_insurance_verifier = Blueprint('identity_insurance_verifier', __name__)

@identity_insurance_verifier.route("/api/identity-insurance/presentation-request", methods = ['GET'])
def presentationRequest():
    ...
    payload["callback"]["url"] = str(request.url_root).replace("http://", "https://") + "api/identity-insurance/presentation-request-callback"
...
@identity_insurance_verifier.route("/api/identity-insurance/presentation-request-callback", methods = ['POST'])
def presentationRequestApiCallback():
    ...
    if presentationResponse["requestStatus"] == "presentation_verified":
        cacheData = {
            ...
            "citizenId": presentationResponse["verifiedCredentialsData"][0]["claims"]["citizenId"],
            "insuranceId": presentationResponse["verifiedCredentialsData"][1]["claims"]["insuranceId"],
            "insuranceType": presentationResponse["verifiedCredentialsData"][1]["claims"]["insuranceType"],
            "insuranceStatus": presentationResponse["verifiedCredentialsData"][1]["claims"]["status"],
...
@identity_insurance_verifier.route("/api/identity-insurance/presentation-response", methods = ['GET'])
...
@identity_insurance_verifier.route("/api/identity-insurance/presentation-response-b2c", methods = ['POST'])
