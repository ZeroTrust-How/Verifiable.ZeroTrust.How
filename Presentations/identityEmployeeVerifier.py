...
if presentationFile is None:
    presentationFile = os.path.realpath(os.path.join(os.path.dirname(__file__), '../Config/identity-employee_presentation.json'))
...
identity_employee_verifier = Blueprint('identity_employee_verifier', __name__)

@identity_employee_verifier.route("/api/identity-employee/presentation-request", methods = ['GET'])
def presentationRequest():
    ...
    payload["callback"]["url"] = str(request.url_root).replace("http://", "https://") + "api/identity-employee/presentation-request-callback"
...
@identity_employee_verifier.route("/api/identity-employee/presentation-request-callback", methods = ['POST'])
def presentationRequestApiCallback():
    ...
    if presentationResponse["requestStatus"] == "presentation_verified":
        cacheData = {
            ...
            "employeeId": presentationResponse["verifiedCredentialsData"][1]["claims"]["employeeId"],
            "employeeStatus": presentationResponse["verifiedCredentialsData"][1]["claims"]["status"],
            "workEmailAddress": presentationResponse["verifiedCredentialsData"][1]["claims"]["emailAddress"],
...
@identity_employee_verifier.route("/api/identity-employee/presentation-response", methods = ['GET'])
...
@identity_employee_verifier.route("/api/identity-employee/presentation-response-b2c", methods = ['POST'])
